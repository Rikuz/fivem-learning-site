...
### 4. `practice.html` — 実践演習一覧
- `assets/js/exercises-data.js` の `EXERCISES` 配列を情報源とし、難易度順(1〜5)にカード一覧を表示する。
- 難易度バッジは`window.TIER_INFO`(🟢入門〜⚫上級)をそのまま流用し、学習の難易度感覚と統一する。
- 各カードから `practice/*.html` の演習詳細ページへ遷移する。

### 5. `practice/*.html` — 演習詳細ページ
- レッスンとは異なり、**完成コードを載せない設計仕様ページ**。学習者はここに書かれた仕様を読んで自分でコードを書く。
- 構成は「概要 → ディレクトリ構造 → 処理仕様 → 期待する動作 → 利用する依存関係 → 関連レッスン」の順で固定(`docs/EXERCISE_OUTLINE.md`参照)。
- 「関連レッスン」セクションで、その演習の実装に必要な既存レッスンへ直接リンクする(演習→レッスンの一方向リンク)。
