...
- `login.html`にはこのスクリプトを含めない(含めると無限リダイレクトになる)。資格情報は平文では保持せず、`assets/js/sha256.js`(依存なしの自前SHA-256実装。Web Crypto API の`crypto.subtle`は`file://`で使えない環境があるため使用しない)でハッシュ化した上で`login.html`内の`CREDENTIAL_HASH`定数(`sha256("ユーザー名:パスワード")`)と比較する。現状のユーザー名・パスワードは`rikuz` / `1234`。**これはページソースを読んだだけでは判別できないようにする目的のみの軽い難読化であり、暗号学的に安全な認証ではない(弱いパスワードはハッシュから総当たりで割り出せる)。** 変更する場合は新しい`"ユーザー名:パスワード"`文字列のSHA-256ハッシュを計算し直し、`CREDENTIAL_HASH`を置き換えること。
